...
export default function Home() {
  ...
  useEffect(() => {
    // Check for guest mode flag in sessionStorage
    const isGuest = sessionStorage.getItem('isGuest') === 'true';

    // If auth is done loading, user is not logged in, AND they are not a guest, redirect to login
  ...
  // If still no user and not a guest (e.g., direct navigation), show loader before potential redirect
  if (!user && (typeof window !== 'undefined' && sessionStorage.getItem('isGuest') !== 'true')) {
    return (
        <div className="flex min-h-screen items-center justify-center">
            <Loader2 className="h-12 w-12 animate-spin text-primary" />
        </div>
